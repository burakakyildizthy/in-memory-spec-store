...
public class StaticMetaModelGenerator {
    ...
    private void addImportsForField(Set<String> imports, VariableElement field) {
        ...
        // Handle collection types - add base Collection import
        if (isCollectionType(fieldType)) {
            imports.add(JAVA_UTIL_COLLECTION);
            debugLog("Added Collection import for field: " + fieldName);

            // If the collection's element type is itself a collection-shaped java.util
            // type (e.g., List<List<String>> -> element type erases to List), that type
            // is referenced by simple name in the generated CollectionAttribute<Owner, ElementType>
            // declaration and therefore needs its own explicit import.
            String erasedElementType = getCollectionElementType(fieldType);
            if (erasedElementType != null && erasedElementType.startsWith("java.util.")) {
                imports.add(erasedElementType);
                debugLog("Added import for nested collection element type: " + erasedElementType);
            }
        }

        // Recursively collect user-type imports from the field type and ALL its type arguments
        collectTypeImports(fieldType, imports);
    }

    /**
     * Recursively collects user-type imports from a type and all of its type arguments.
     * The import decision (whether to add a type to imports) is independent of the
     * recursion decision (always recurse into type arguments).
     */
    private void collectTypeImports(TypeMirror type, Set<String> imports) {
        if (type instanceof DeclaredType declaredType) {
            Element element = declaredType.asElement();
            if (element instanceof TypeElement typeElement) {
                String qualifiedName = typeElement.getQualifiedName().toString();

                // Import decision: add only user types (same predicate as existing code)
                if (!PRIMITIVE_TYPE_MAPPING.containsKey(qualifiedName) &&
                    ...
                    debugLog("Added type import: " + qualifiedName);
                }
            }

            // Recursion decision: ALWAYS recurse into ALL type arguments
            // regardless of whether this type itself was imported
    ...
    private void generateCollectionAttribute(PrintWriter out, VariableElement field, String ownerClassName) {
        ...
        String declaredCollectionType = getDeclaredCollectionType(field.asType());

        out.println("    public static final CollectionAttribute<" + ownerSimpleName + ", " + elementSimpleName + "> " + fieldName + " =");
        out.println("        new CollectionAttribute<>(\"" + fieldName + "\", " + ownerSimpleName + CLASS_SUFFIX2 + elementSimpleName + CLASS_SUFFIX2 + declaredCollectionType + CLASS_SUFFIX);
    }

    /**
     * Determines the declared collection type (List, Set, or Collection) from a field's type.
     * Maps concrete implementations (ArrayList, HashSet, etc.) to their interface counterparts.
     * Only List, Set, and Collection are supported per MetaModelValidator.
     *
     * @param fieldType the field's TypeMirror
     * @return the fully qualified collection interface name (e.g., "java.util.Set")
     */
    private String getDeclaredCollectionType(TypeMirror fieldType) {
        if (fieldType instanceof DeclaredType declaredType) {
            ...
            if (element instanceof TypeElement typeElement) {
                ...
                // Concrete implementations → map to interface
                if (typeName.equals("java.util.ArrayList") || typeName.equals("java.util.LinkedList")) {
                    return "java.util.List";
                }
                if (typeName.equals("java.util.HashSet") || typeName.equals("java.util.TreeSet")) {
                    return "java.util.Set";
                }

                // Check type hierarchy: is it assignable to Set or List?
                TypeMirror setType = processingEnv.getElementUtils()
                        .getTypeElement("java.util.Set").asType();
                TypeMirror listType = processingEnv.getElementUtils()
                        .getTypeElement("java.util.List").asType();

                TypeMirror erasedFieldType = processingEnv.getTypeUtils().erasure(fieldType);
                if (processingEnv.getTypeUtils().isAssignable(erasedFieldType,
                        processingEnv.getTypeUtils().erasure(setType))) {
                    return "java.util.Set";
                }
                if (processingEnv.getTypeUtils().isAssignable(erasedFieldType,
                        processingEnv.getTypeUtils().erasure(listType))) {
                    return "java.util.List";
                }
            }
        }
        // Fallback: Collection (safest default)
        return JAVA_UTIL_COLLECTION;
    }
    ...
    private String getCollectionElementType(TypeMirror collectionType) {
        if (collectionType instanceof DeclaredType declaredType) {
            ...
            if (!typeArguments.isEmpty()) {
                ...
                // Only erase when the element type argument is ITSELF parameterized
                // (e.g., List<String> inside List<List<String>>), since a Class<E>
                // reference cannot carry generic type information in that case.
                // For simple, non-generic element types (e.g., Order in List<Order>),
                // erasure is unnecessary and skipped to preserve exact prior behavior.
                if (elementTypeMirror instanceof DeclaredType elementDeclaredType
                        && !elementDeclaredType.getTypeArguments().isEmpty()) {
                    TypeMirror erased = processingEnv.getTypeUtils().erasure(elementTypeMirror);
                    return erased.toString();
                }
                return elementTypeMirror.toString();
            }
        }
